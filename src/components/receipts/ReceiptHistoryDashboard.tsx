...
export function ReceiptHistoryDashboard({ onScanNewReceipt, onEditReceipt, currentAdminUser = "admin" }: ReceiptHistoryDashboardProps) {
  ...
  const _handleSettleReceiptRequest = async (receipt: ReceiptData) => {
  ...
  const filteredReceipts = useMemo(() => {
    ...
    const sevenDaysAgoDate = new Date(d.getFullYear(), d.getMonth(), d.getDate() - 7)
  ...
  const _topProductsData = useMemo(() => {
  ...
  const _handleOpenProductBreakdown = (productName: string) => {
    ...
    setItemBreakdownModal({
      title: productName,
      subTitle: `Kategori Utama: ${catName} â€¢ Sub-Kategori: ${subCatName}`,
      totalSpend,
      totalQty,
                  ...
                  {subCategoryChartData.slice(0, 5).map((prod, idx) => (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-slate-500 font-medium truncate">
                        {prod.count} item â€¢ [{prod.category} / {prod.subCategory}]
                      </p>
                ...
                <div className="space-y-1">
                  ...
                  <div className="space-y-0.5 text-[11px] font-semibold text-slate-800 pt-0.5 leading-tight">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>:</span>
                      <span className="font-bold font-mono text-emerald-700">{statementTableRows.length} Struk â€” Rp {totalSpend.toLocaleString("id-ID")}</span>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-md p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 text-xs space-y-1">
              ...
              <p className="text-slate-600">
                Tanggal: {deletingReceipt.date} â€¢ Total:{" "}
                          ...
                          <div className="p-4 bg-slate-50/90 border-t border-slate-200 space-y-3 text-xs animate-in fade-in zoom-in-98 duration-150">
                            ...
                            {reqItem.actionType === "EDIT" && (
                              <div className="bg-white p-3 rounded-xl border border-slate-200 space-y-2 text-xs">
                                <span className="font-black text-slate-900 uppercase tracking-wider text-[11px] block border-b border-slate-100 pb-1">
                                  ðŸ” Perbandingan Perubahan Data Edit:
                                </span>
                                      ...
                                      <div className="pt-1 border-t border-slate-200">
                                        ...
                                        <ul className="list-disc pl-3 text-[10.5px] space-y-0.5 text-slate-600">
                                          {targetReceipt.items.map((i: any, idx: number) => (
                                            <li key={idx}>{i.name} (x{i.quantity}) â€” Rp {Number(i.price * i.quantity).toLocaleString("id-ID")}</li>
                                          ))}
                                        </ul>
                                      </div>
                                    )}
                                      ...
                                      <div className="pt-1 border-t border-emerald-200">
                                        ...
                                        <ul className="list-disc pl-3 text-[10.5px] space-y-0.5 text-emerald-900 font-medium">
                                          {payloadObj.items.map((i: any, idx: number) => (
                                            <li key={idx}>{i.name} (x{i.quantity || 1}) â€” Rp {Number((i.price || 0) * (i.quantity || 1)).toLocaleString("id-ID")}</li>
                                          ))}
                                        </ul>
                                      </div>
                                    )}
                            ...
                            {(reqItem.actionType === "DELETE" || reqItem.actionType === "BULK_DELETE") && (
                              <div className="bg-red-50 p-3 rounded-xl border border-red-200 text-xs text-red-900 space-y-1">
                                <span className="font-bold block">âš ï¸ Peringatan Penghapusan Nota:</span>
