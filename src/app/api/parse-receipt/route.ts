...
export async function POST(req: NextRequest) {
  try {
    ...
    const { imageBase64, ocrText, customApiKey } = body;
    ...
    if (!apiKey) {
      return NextResponse.json(
        {
          error: "CONFIG_ERROR",
          message: "GEMINI_API_KEY belum dikonfigurasi. Silakan atur di .env atau Pengaturan.",
        },
        { status: 400 }
      );
    }

    const rawText = sanitizeRawText(ocrText || "");

    // 2. Fetch Category Hierarchy from Supabase
    const categoryHierarchy = await getOrSeedCategories();

    const officialHierarchyMap = categoryHierarchy.map((h) => ({
      parentName: h.name,
      subNames: h.subCategories.map((s) => s.name),
    }));

    let officialCategoriesPromptText = "DAFTAR HIERARKI KATEGORI & SUB-KATEGORI RESMI (PILIH HANYA DARI DAFTAR INI):\n";
    officialHierarchyMap.forEach((h) => {
      officialCategoriesPromptText += `- Kategori Utama: "${h.parentName}" -> Sub-Kategori yang diperbolehkan: [${h.subNames.map((s) => `"${s}"`).join(", ")}]\n`;
    });

    // 3. Retrieve Self-Learning Knowledge Context from Supabase
...
`;
    ...
    if (!textOutput) {
      if (lastError?.status === 429 || lastError?.message === "GOOGLE_CLOUD_QUOTA_EXCEEDED") {
        return NextResponse.json(
          {
            error: "QUOTA_EXCEEDED",
            message: "Batas frekuensi Google Gemini API tercapai. Silakan coba beberapa detik lagi.",
          },
          { status: 429 }
        );
      }
    ...
    parsedJson.items = await Promise.all(
      parsedJson.items.map(async (it) => {
        ...
        const targetCat = localMatch ? localMatch.category : (it.category || "").trim();
        const targetSub = localMatch ? localMatch.subCategory : (it.subCategory || "").trim();
        ...
        const itemSku = (it as any).sku || (localMatch as any)?.sku || generateItemSku(rawItemName, finalParentCategory);
